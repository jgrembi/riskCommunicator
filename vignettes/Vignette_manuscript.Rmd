...
### Ease of plotting output with ggplot2 - just for gComp results
...
### Ease of plotting output with ggplot2 - comparing against standard regression model results
```{r plot-rate-outcome-results-v-std-regression, warning=F}
#saving the output and modifying the labels of the SEX variable to specify male/female instead of 0/1
# also adding a new variable to show the line indicating no difference found (0 for rate difference, 1 for rate ratio)
df.standard.regression = data.frame(Subgroup = c("Male", "Male", "Female", "Female"), Parameter = c("Incidence Rate Difference", "Incidence Rate Ratio", "Incidence Rate Difference", "Incidence Rate Ratio"), Estimate = c(NA, 1.79, NA, 2.04), lCI = c(NA, 1.32, NA, 1.51), uCI = c(NA, 2.44, NA, 2.77)) %>%
  mutate(outcome = "cvd_dth", 
         Comparison = "DIABETES1_v._DIABETES0",
         hline = ifelse(Parameter == "Incidence Rate Ratio", 1, 0),
         calc = "Standard regression model - covariate-conditional effect estimate") %>%
  rename(`97.5% CL` = uCI, `2.5% CL` = lCI)
df2 = rate.res.group$results.df %>%
  mutate(Subgroup = ifelse(Subgroup == "SEX0", "Male", "Female"),
         hline = ifelse(Parameter == "Incidence Rate Ratio", 1, 0),
         calc = "riskCommunicator - marginal effect estimate") %>%
  bind_rows(df.standard.regression)

ggplot(df2, aes(x = Subgroup, y = Estimate, color = calc)) + 
