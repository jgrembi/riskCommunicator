...
Soon, you will be able to install the released version of `riskCommunicator` from [CRAN](https://CRAN.R-project.org) with:

``` r
install.packages("riskCommunicator")
```

For now, the development version is available as a source package through [GitHub](https://github.com/).  Installation requires the ability to compile R packages. This means that R and the R tool-chain must be installed, which requires the Xcode command-line tools on Mac and Rtools on Windows.
